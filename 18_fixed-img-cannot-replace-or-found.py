"""
Universal Manager V18 - Versión 18 (No-Lock & Exact Files)
- Fix: "Archivo en uso": Las imágenes se cargan en memoria (BytesIO) para liberar el archivo inmediatamente.
- Fix: Eliminadas las extensiones inteligentes. Se usa el nombre de archivo literal.
- Fix: Al subir imagen, se usa siempre el nombre completo con extensión (reemplazo básico).
"""
import sys
import os
import subprocess
...
class UniversalDBManagerV18:
    def __init__(self, root):
        self.root = root
        self.root.title("Tarquitet - Universal Manager V18 (No-Lock)")
                ...
                if k in ['fileName', 'image']:
                    ...
                    def pick_file(entry=ent):
                        ...
                        if f_path:
                            ...
                            if is_external:
                                def on_path_selected(target_folder):
                                    try:
                                        ...
                                        shutil.copy2(f_path, dst) # Copia simple y directa
                            ...
                            # UPDATE V18: Siempre usar nombre completo con extensión
    ...
    def _img_th(self, fn):
        ...
        if str(fn).startswith("http"):
            ...
            # UPDATE V18: Búsqueda exacta, sin adivinar extensiones
            found = None
            for p in self.image_search_paths:
                f_path = os.path.join(p, clean) # Nombre literal
                if os.path.exists(f_path): 
                    found = f_path; break
            
            if found:
                try:
                    # UPDATE V18: Cargar en memoria para liberar el archivo (evita error File In Use)
                ...
                self.root.after(0, lambda: self.show_img(None, "No encontrado (Nombre exacto)"))
