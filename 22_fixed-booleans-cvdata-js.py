...
class UniversalDBManagerV24:
        ...
        for item in data:
            ...
            for k, v in item.items():
                if isinstance(v, list):
                    ...
                    s = str(v).strip()
                    if s.startswith(prefixes) or s in consts: 
                        lines.append(f"    {k}: {s},")
                    # NUEVO: Detecta booleanos y los escribe SIN comillas
                    elif s.lower() in ['true', 'false']:
                        lines.append(f"    {k}: {s.lower()},")
                    else: 
                        lines.append(f"    {k}: '{s.replace('\'', '\\\'')}',")
            lines.append("  },")
        return "\n".join(lines)
    ...
    def py_to_js_obj(self, data):
        ...
        for k, v in data.items():
            ...
            s = str(v).strip()
            # Si es booleano, va sin comillas
            if s.lower() in ['true', 'false']:
                lines.append(f"  {key_str}: {s.lower()},")
            else:
                lines.append(f"  {key_str}: '{s}',")
        return "\n".join(lines)
