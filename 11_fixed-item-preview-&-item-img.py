...
class JSParser:
    ...
    def parse_kv_inside_object(raw_content):
        data = {}
        # DOTALL para permitir valores multilinea
    ...
    def parse_array_of_objects(raw_array):
        ...
        # Split por '},' o '} ,' permitiendo saltos de línea
...
class ListEditorDialog(ttk.Toplevel):
    def __init__(self, parent, title, current_list, all_options, callback):
        super().__init__(parent); self.title(title); self.geometry("450x600")
        ...
        ttk.Label(self, text=f"{title}", font=("Arial", 10, "bold")).pack(pady=10)
        ...
        for opt in self.all_options:
            v = tk.BooleanVar(value=opt in safe_current); self.vars[opt] = v
...
class ConstantsEditor(ttk.Toplevel):
    def __init__(self, parent, constants_data, callback):
        super().__init__(parent); self.title("Editor Constantes"); self.geometry("700x600")
...
# ============================================================
# APP PRINCIPAL (V18 - IMAGE MASTER)
# ============================================================
class UniversalDBManagerV18:
    def __init__(self, root):
        self.root = root
        self.root.title("Tarquitet - Universal Manager V18 (Image Master)")
        ...
        self.constants = {}
        self.schemas = {}
        self.image_search_paths = [] # Rutas escaneadas
    ...
    def load_file(self, path):
        try:
            ...
            # --- SCAN DE RUTAS DE IMÁGENES ---
            self.scan_image_paths()
            self.refresh_tabs()
            self.image_cache = {}
        except Exception as e: messagebox.showerror("Error Carga", str(e))

    def scan_image_paths(self):
        # 1. Rutas básicas relativas al JS
        ...
        # 2. Escaneo profundo de PROJECT_CONFIG si existe
        # Buscamos cadenas tipo '../assets/...' dentro del archivo crudo
        ...
        # 3. Auto-descubrir subcarpetas (dev, design, etc) en rutas existentes
        subfolders = []
        for p in self.image_search_paths:
            if os.path.exists(p):
                try: 
                    # Listar directorios dentro
        ...
        print("Rutas de imagen:", self.image_search_paths)
    ...
    def get_suggestions(self, key):
        ...
        if key in map_c and map_c[key] in self.constants:
            ...
            for k in self.constants[c_name]:
                opts.add(f"{c_name}.{k}"); opts.add(self.constants[c_name][k])
    ...
    def build_form(self, item):
        ...
        for k in sorted_keys:
            ...
            if is_list:
                ...
                # --- MANEJO DE CAMPO DE IMAGEN (FILE SELECTOR) ---
                if k in ['fileName', 'image']:
                    ...
                    def pick_file(entry=ent):
                        ...
                        if f:
                            # Lógica inteligente: mantenemos el comportamiento de guardado
                            # (usar nombre o nombre sin extensión según objetos existentes),
                            # pero forzamos la vista previa usando la ruta absoluta seleccionada.
                            ...
                            if "." in sample:
                                ...
                                entry.delete(0, tk.END)
                                entry.insert(0, name_no_ext)

                            # Guardar en memoria (actualiza dataset) pero además
                            # forzamos la carga inmediata del preview apuntando
                            # a la ruta absoluta seleccionada para evitar problemas
                            # cuando la imagen no esté en las rutas preconfiguradas.
                            self.save_memory()
                            # preview explícito con la ruta absoluta
                            try:
                                self.load_image({'fileName': f})
                            except Exception:
                                pass
                    ...
                    # Guardar al escribir para que los cambios se reflejen en tiempo real
                    ent.bind("<KeyRelease>", lambda e: self.save_memory())
                    if not is_present: 
                        ent.configure(state="disabled")
                        btn_br.configure(state="disabled")
                    ...
                    if sug:
                        # Aseguramos que el combobox sea editable para permitir escritura directa
    ...
    def save_memory(self):
        ...
        # Recargar imagen si cambió el filename
