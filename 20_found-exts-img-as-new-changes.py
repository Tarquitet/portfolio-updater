"""
Universal Manager V20 - Auto-Fix Extensions & No-Lock
- Feature: "Auto-Fix": Si el nombre no tiene extensión y el escáner encuentra la imagen,
  ACTUALIZA AUTOMÁTICAMENTE el campo de texto y la memoria (ej: 'foto' -> 'foto.png').
- Fix: Mantiene el desbloqueo de archivos y la carga segura.
"""
import sys
import os
import subprocess
...
class ListEditorDialog(ttk.Toplevel):
    def __init__(self, parent, title, current_list, all_options, callback):
        ...
        for opt in self.all_options: self.add_checkbox(opt, opt in safe_current)
...
class PathChooserDialog(ttk.Toplevel):
    def __init__(self, parent, paths, filename, callback):
        ...
        ttk.Label(self, text="Selecciona dónde quieres guardarla:", justify="center").pack(pady=5)
        ...
        # Limpiar y mostrar rutas únicas
...
class ConstantsEditor(ttk.Toplevel):
    def __init__(self, parent, constants_data, callback):
        ...
        ttk.Button(self, text="ACTUALIZAR EN MEMORIA", command=self.save, bootstyle="success").pack(fill="x", padx=10, pady=10)
...
class UniversalDBManagerV20:
    def __init__(self, root):
        self.root = root
        self.root.title("Tarquitet - Universal Manager V20 (Auto-Fix)")
    ...
    def build_ui(self):
        ...
        ttk.Label(h, text="UNIVERSAL DATA V20", font=("Helvetica", 16, "bold"), bootstyle="primary").pack(side="left")
                            ...
                            if is_external:
                                def on_path_selected(target_folder):
                                    ...
                                    try:
                                        ...
                                        messagebox.showerror("Archivo Bloqueado", f"Windows no permite sobreescribir:\n{dst}\n\nEspera unos segundos o cierra otros programas.")
                                    except Exception as ex:
                                        messagebox.showerror("Error al copiar", str(ex))
    ...
    # --- MÉTODO PARA ACTUALIZAR UI DESDE EL THREAD ---
    def auto_fix_extension(self, key_name, found_name):
        # Esta función se llama si encontramos la imagen correcta pero el texto estaba incompleto
        # Actualizamos el widget y la memoria
        if key_name in self.form_widgets and self.form_widgets[key_name]['type'] == 'text':
            ...
            # Solo actualizamos si es diferente (ej: 'foto' != 'foto.png')
            if current_val != found_name:
                widget.delete(0, tk.END)
                widget.insert(0, found_name)
                # Forzamos guardado en memoria
                self.save_memory()
                print(f"Auto-Fix: {current_val} -> {found_name}")
            ...
            if prev_key and prev_ident and prev_key in self.datasets:
                for idx, itm in enumerate(self.datasets[prev_key]):
                    if itm.get('title') == prev_ident or itm.get('fileName') == prev_ident:
                        self.var_tab.set(prev_key); self.load_list(); self.listbox.select_set(idx); self.on_select(None)
                        break
    ...
    def edit_constants(self):
        ...
        def save(new_c): 
            ...
            messagebox.showinfo("Memoria Actualizada", "Constantes actualizadas en memoria.\nRecuerda GUARDAR CAMBIOS para persistir.")
    ...
    def _img_th(self, fn, key_used):
        # UPDATE V20: Recibimos key_used para saber qué campo actualizar
        ...
        if str(fn).startswith("http"):
            ...
            # --- V20 SMART SCAN & AUTO-FIX ---
            found = None
            extensions_to_try = ['', '.png', '.jpg', '.jpeg', '.webp', '.avif', '.gif']
            ...
            if found:
                try:
                    ...
                    self.show_img(pil_img, found)
                    
                    # === AUTO FIX MAGIC ===
                    # Si el nombre original NO tenía extensión (o era diferente) y encontramos uno válido:
                    # Actualizamos el campo en la UI
                    found_basename = os.path.basename(found)
                    if found_basename != clean:
                         self.root.after(0, lambda: self.auto_fix_extension(key_used, found_basename))

                except Exception as e:
                    print(e)
                    self.root.after(0, lambda: self.show_img(None, "Error Lectura"))
            else: 
                self.root.after(0, lambda: self.show_img(None, "No encontrado (Escaneado)"))
...
if __name__ == "__main__":
    ...
    app = UniversalDBManagerV20(app_window)
    app_window.mainloop()