"""
Universal Manager V18 - Versión 17 (Fixed Form & Constants)
- Fix: Dropdowns restringidos a category y context.
- Fix: Guardado de constantes no actualiza el archivo (evita disparar manager.py).
- Fix: Recarga de constantes en memoria (tools actualizadas inmediatamente).
- Fix: Guardado de inputs al presionar Enter (<Return>).
"""
import sys
import os
import subprocess
...
class PathChooserDialog(ttk.Toplevel):
    def __init__(self, parent, paths, filename, callback):
        ...
        ttk.Button(btn_frame, text="CANCELAR (Solo vincular)", command=self.destroy, bootstyle="secondary").pack(side="left", expand=True)
...
class UniversalDBManagerV18:
    def __init__(self, root):
        self.root = root
        self.root.title("Tarquitet - Universal Manager V18 (Fixed Form)")
    ...
    def load_file(self, path):
        try:
            ...
            self.filepath = os.path.abspath(path) # Asegurar ruta absoluta
    ...
    def scan_image_paths(self):
        ...
        # Buscar rutas en el archivo JS (regex)
        paths_found = re.findall(r"['\"](\.\./assets/.*?)['\"]", self.file_content)
        for p in paths_found:
            # Construir ruta absoluta y normalizar
                ...
                if k in ['fileName', 'image']:
                    ...
                    def pick_file(entry=ent):
                        ...
                        if f_path:
                            ...
                            # Lógica: Si el archivo NO empieza con la ruta base del proyecto, es externo.
                            ...
                            if is_external:
                                def on_path_selected(target_folder):
                                    try:
                                        ...
                                        shutil.copy2(f_path, dst) # copy2 preserva metadatos
                    ...
                    # CORRECCIÓN 4: Guardar en memoria al hacer Enter o FocusOut
                    ...
                    # CORRECCIÓN 1: Dropdown solo para category y context
    ...
    def edit_constants(self):
        ...
        # CORRECCIÓN 2 y 3: No guardar en archivo automáticamente para no disparar manager.py
        def save(new_c): 
            self.constants = new_c
            messagebox.showinfo("Memoria Actualizada", "Constantes actualizadas en la memoria.\nLas listas de 'Tools' ya pueden usar los nuevos valores.\n\n⚠️ IMPORTANTE: No olvides presionar 'GUARDAR CAMBIOS' en la pantalla principal para escribirlas en el archivo.")
            
            # Recargar el formulario actual para que 'tools' cargue las nuevas constantes
