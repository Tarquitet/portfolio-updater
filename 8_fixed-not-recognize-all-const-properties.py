...
# ============================================================
# PARSER ROBUSTO V14 (CON LIMPIEZA DE COMENTARIOS)
# ============================================================
class JSParser:
    @staticmethod
    def remove_comments(text):
        """Elimina comentarios /* */ y // de forma segura, respetando URLs y strings."""
        def replacer(match):
            s = match.group(0)
            if s.startswith('/'):
                return " " # Reemplazar comentario por espacio
            else:
                return s # Dejar string intacto
        
        # Regex que captura: Strings ("..." o '...') O Comentarios (//... o /*...*/)
        pattern = re.compile(
            r'//.*?$|/\*.*?\*/|\'(?:\\.|[^\\\'])*\'|"(?:\\.|[^\\"])*"',
    ...
    def extract_objects(content):
        # 1. Limpieza inicial de todo el archivo
        clean_content = JSParser.remove_comments(content)
        objects = {}
        
        # Regex para encontrar declaraciones: const X =, window.X =
        regex_start = re.compile(r"(?:const\s+|let\s+|var\s+|window\.)\s*(\w+)\s*=\s*([\[\{])")
        
        for match in regex_start.finditer(clean_content):
            var_name = match.group(1)
            start_char = match.group(2)
            start_idx = match.start(2)
            
            # Algoritmo de conteo de llaves (Brace Counting)
            balance = 0
            end_idx = -1
            in_string = False
            string_char = ''
            ...
            for i in range(start_idx, len(clean_content)):
                ...
                if char in ["'", '"', "`"] and (i == 0 or clean_content[i-1] != "\\"):
                    if not in_string:
                        in_string = True
                        string_char = char
            ...
            if end_idx != -1:
                objects[var_name] = clean_content[start_idx:end_idx]
    ...
    def parse_kv_inside_object(raw_content):
        data = {}
        # Regex: Clave : Valor (String)
        # Soporta claves normales (key:) y computadas ([KEY]:)
        regex_prop = re.compile(r"(\w+|\[.*?\])\s*:\s*(?:'([^']*)'|\"([^\"]*)\")")
    ...
    def parse_array_of_objects(raw_array):
        ...
        for raw in raw_objs:
            clean = raw.strip()
            # Reconstruir llaves perdidas en el split
            ...
            # 2. Arrays (tools: [...])
            regex_arrays = re.compile(r"(\w+)\s*:\s*\[(.*?)\]")
            ...
            # 3. Constantes (category: CAT.DEV)
            # Busca valores sin comillas que parezcan variables
            regex_const = re.compile(r"(\w+)\s*:\s*(?!['\"\[])([a-zA-Z0-9_.]+)(?!['\"\]])")
            for m in regex_const.finditer(clean):
                k = m.group(1)
                v = m.group(2)
...
class ListEditorDialog(ttk.Toplevel):
    def __init__(self, parent, title, current_list, all_options, callback):
        ...
        self.title(title); self.geometry("450x600"); self.callback = callback
        
        clean_opts = set([x.replace("'","").replace('"',"").strip() for x in all_options + current_list if x])
        self.all_options = sorted(list(clean_opts))
        self.vars = {}

        ttk.Label(self, text=f"Editar Lista: {title}", font=("Arial", 10, "bold")).pack(pady=10)
        
        c = tk.Canvas(self, highlightthickness=0); sb = ttk.Scrollbar(self, orient="vertical", command=c.yview)
        f = ttk.Frame(c); f.bind("<Configure>", lambda e: c.configure(scrollregion=c.bbox("all")))
        c.create_window((0,0), window=f, anchor="nw"); c.configure(yscrollcommand=sb.set)
        c.pack(side="left", fill="both", expand=True, padx=10); sb.pack(side="right", fill="y")
        
        c.bind_all("<MouseWheel>", lambda e: c.yview_scroll(int(-1*(e.delta/120)), "units"))
        
        active_clean = [x.replace("'","").replace('"',"").strip() for x in current_list]
        for opt in self.all_options:
            v = tk.BooleanVar(value=opt in active_clean); self.vars[opt] = v
            ttk.Checkbutton(f, text=opt, variable=v).pack(anchor="w", pady=2)

        af = ttk.Frame(self, padding=10); af.pack(fill="x")
        self.en = ttk.Entry(af); self.en.pack(side="left", fill="x", expand=True, padx=5)
        self.en.bind("<Return>", lambda e: self.add())
        ttk.Button(af, text="Añadir", command=self.add).pack(side="right")
        
        ttk.Button(self, text="GUARDAR", command=self.save, bootstyle="success").pack(fill="x", padx=10, pady=10)

    def add(self):
        val = self.en.get().strip()
        if val: self.vars[val] = tk.BooleanVar(value=True); self.save()
    def save(self):
        self.callback([k for k,v in self.vars.items() if v.get()]); self.destroy()

class ConstantsEditor(ttk.Toplevel):
    def __init__(self, parent, constants_data, callback):
        super().__init__(parent); self.title("Editor de Constantes"); self.geometry("700x600")
        self.data = constants_data; self.callback = callback; self.entries = {}
...
# APP PRINCIPAL (V14)
# ============================================================
class UniversalDBManagerV14:
    def __init__(self, root):
        self.root = root
        self.root.title("Tarquitet - Universal Manager V14 (Final Fix)")
    ...
    def build_ui(self):
        ...
        ttk.Label(h, text="UNIVERSAL DATA V14", font=("Helvetica", 16, "bold"), bootstyle="primary").pack(side="left")
        ...
        self.tabs_frame = ttk.Labelframe(left, text="Datasets Detectados", padding=5); self.tabs_frame.pack(fill="x")
        self.lbl_nodata = ttk.Label(self.tabs_frame, text="Sin datos"); self.lbl_nodata.pack()
        ...
        self.c.bind_all("<MouseWheel>", lambda e: self.c.yview_scroll(int(-1*(e.delta/120)), "units"))
    ...
    def load_file(self, path):
            ...
            for name, raw_content in raw_objects.items():
                if raw_content.strip().startswith('['):
                    ...
                    if data:
                        ...
                        keys = set()
                        for i in data: keys.update(i.keys())
                        self.schemas[name] = list(keys)
        ...
        except Exception as e: messagebox.showerror("Error de Carga", str(e))

    def refresh_tabs(self):
        for w in self.tabs_frame.winfo_children(): w.destroy()
        if not self.datasets: 
            self.lbl_nodata = ttk.Label(self.tabs_frame, text="No se encontraron arrays."); self.lbl_nodata.pack()
            return
        ...
        for k in self.datasets.keys():
            ttk.Radiobutton(self.tabs_frame, text=k, variable=self.var_tab, value=k, 
                           command=self.load_list, bootstyle="toolbutton-outline").pack(side="left", fill="x", expand=True)
        self.load_list()
    ...
    def get_suggestions(self, key):
        ...
        if key in map_c and map_c[key] in self.constants:
            c_name = map_c[key]
            for k in self.constants[c_name]:
                opts.add(f"{c_name}.{k}")
                opts.add(self.constants[c_name][k])
    ...
    def build_form(self, item):
        ...
        for k in sorted_keys:
            ...
            ttk.Checkbutton(row, text=k.upper(), variable=chk_var, bootstyle="round-toggle", command=toggle).pack(side="left", width=15)
            ...
            if is_list:
                ...
                if sug:
                    ent = AutocompleteCombobox(row, all_options=sug)
                    ent.set(str(val))
                else:
                    ent = ttk.Entry(row)
    ...
    def edit_constants(self):
        ...
        def save(new_c):
            self.constants = new_c
            self.save_file(); self.smart_load()
    ...
    def load_image(self, item):
        ...
        if not fn: 
            self.lbl_img.config(image='', text="Sin Imagen"); self.lbl_path.config(text="")
            return
    ...
    def _img_th(self, fn):
        if fn.startswith("http"): 
            ...
            # Buscar en config paths si existen
            paths = [os.path.join(base, "../assets/images")]
            if "PROJECT_CONFIG" in self.constants:
                pc = self.constants["PROJECT_CONFIG"]
                # Intento básico de extraer rutas de PROJECT_CONFIG
                # (Aquí se podría mejorar leyendo paths dentro)
                pass
            ...
            else: self.root.after(0, lambda: self.show_img(None, "No encontrado"))
...
if __name__ == "__main__":
    ...
    app = UniversalDBManagerV14(app_window)
    app_window.mainloop()