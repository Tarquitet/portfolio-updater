"""
Universal Manager V18 - Versión 12
- Arreglos principales:
  - Evita `bind_all` en canvases para prevenir errores al cerrar dialogs.
  - Scroll ligado a canvas (seguro si el widget existe).
  - Tras `GUARDAR` recarga dinámicamente el JS y preserva la selección.
  - Guarda preview inmediato desde ruta absoluta cuando se selecciona archivo.

Compatibilidad: requiere las mismas dependencias que la versión 11.
"""
import sys
import os
import subprocess
...
# Intentamos mantener compatibilidad con la versión existente y solo
# cambiar lo necesario para los problemas reportados.
...
class ListEditorDialog(ttk.Toplevel):
    def __init__(self, parent, title, current_list, all_options, callback):
        ...
        # Bind scroll directly to the canvas (avoid bind_all global bindings)
        def _wheel(e, canvas=c):
            try:
        ...
        c.bind_all("<MouseWheel>", lambda e: None)  # defensiva: evita que otras binding fallen

        c.bind_all("<Button-4>", lambda e: None)
        c.bind_all("<Button-5>", lambda e: None)

        c.bind_all("<MouseWheel>", lambda e: None)

        for opt in self.all_options:
            v = tk.BooleanVar(value=opt in safe_current)
            self.vars[opt] = v
    ...
    def add(self):
        ...
        if val:
            self.vars[val] = tk.BooleanVar(value=True)
            self.save()
...
class UniversalDBManagerV18:
    def __init__(self, root):
        self.root = root
        self.root.title("Tarquitet - Universal Manager V18 (Image Master) - v12")
    ...
    def build_ui(self):
        ...
        # Bind mousewheel directly to this canvas only (safer que bind_all)
        ...
        for k in sorted_keys:
            ...
            if is_list:
                ...
                if k in ['fileName', 'image']:
                    ...
                    def pick_file(entry=ent):
                        ...
                        if f:
                            ...
                            if "." in sample:
                                ...
                                entry.delete(0, tk.END); entry.insert(0, name_no_ext)
                            self.save_memory()
                            try:
                    ...
                    if not is_present:
                        ent.configure(state="disabled"); btn_br.configure(state="disabled")
    ...
    def save_file(self):
        # Guardar estado para intentar restaurarlo tras reload
        ...
        try:
            ...
            # Recargar dinámicamente y restaurar selección si es posible
